...
function CanvasBackground(config, eventBus, canvas) {
  ...
  function ensureSvgLayer() {
    const container = (canvas && canvas.getContainer) ? canvas.getContainer() : null
    if (!container)
      return
    ...
    let grad = svgRoot.querySelector('#forge-bg-grad')
    if (!grad) {
  ...
  function getViewBox() {
    ...
    if (vbAttr) {
      ...
      const isValidViewBox = parts.length === 4 && parts.every((n) => {
  ...
  function drawDots(vb) {
    ...
    const endX = Math.ceil((vb.x + vb.width) / spacing) * spacing
    const startY = Math.floor(vb.y / spacing) * spacing
    const endY = Math.ceil((vb.y + vb.height) / spacing) * spacing
    for (let x = startX; x <= endX; x += spacing) {
      for (let y = startY; y <= endY; y += spacing) {
        if (x % 100 === 0 && y % 100 === 0) {
          const cMajor = svgCreate('circle')
  ...
  function onDestroy() {
    if (dotsGroup && dotsGroup.parentNode)
      dotsGroup.parentNode.removeChild(dotsGroup)
    if (gradientRect && gradientRect.parentNode)
      gradientRect.parentNode.removeChild(gradientRect)
