...
const emit = defineEmits(['save', 'change', 'ready', 'importStart', 'importEnd', 'selectionChange'])
...
function normalizeImportXml(xml) {
  if (!xml || !xml.trim()) {
...
function fitCanvasWithoutUpscale() {
...
async function importXML(xml) {
  emit('importStart')
  xml = normalizeImportXml(xml)

  try {
    await modeler.importXML(xml)
    selectedElement.value = null
    emit('selectionChange', null)
    fitCanvasWithoutUpscale()
    updateUndoRedoState()
...
watch(() => props.xml, async (newXml) => {
  if (modeler) {
    ...
    const xmlToImport = normalizeImportXml(newXml)
