...
| 2026-06-29 | 补修：列表业务摘要、动态表单权限、代码业务页权限钩子、按钮/变量映射文案 | 空白/冲突检查 | `git diff --check` | 通过 | 无空白错误。 |
| 2026-06-29 | 补修：业务摘要批量补齐 SPI、采购单 Provider 摘要、业务流程上下文 | Maven 编译 | `env JAVA_HOME=/opt/homebrew/Cellar/openjdk@17/17.0.13/libexec/openjdk.jdk/Contents/Home PATH=/opt/homebrew/Cellar/openjdk@17/17.0.13/libexec/openjdk.jdk/Contents/Home/bin:$PATH mvn -pl forge-flow/forge-flow-client,forge-framework/forge-plugin-parent/forge-plugin-generator,forge-business/forge-business-core -am compile -DskipTests` | 通过 | 仅有既有 deprecation/unchecked warning；未跑 JUnit。 |
| 2026-06-29 | 补修：待办/已办表单渲染、代码业务页适配、变量映射文案 | 前端生产构建 | `/bin/zsh -lc "source ~/.nvm/nvm.sh && nvm use v20.19.0 && NODE_OPTIONS=--max-old-space-size=8192 pnpm --dir forge-admin-ui build"` | 通过 | 构建耗时约 1m28s；仅有既有 dynamic/static import chunking、CSS `//` 注释和组件命名冲突 warning。 |
| 2026-06-29 | `forge-plugin-flow` 单模块编译 | Maven 编译 | `env JAVA_HOME=/opt/homebrew/Cellar/openjdk@17/17.0.13/libexec/openjdk.jdk/Contents/Home PATH=/opt/homebrew/Cellar/openjdk@17/17.0.13/libexec/openjdk.jdk/Contents/Home/bin:$PATH mvn -pl forge-framework/forge-plugin-parent/forge-plugin-flow -am compile -DskipTests` | 未通过 | 阻断集中在既有 `FlowFormServiceImpl`：`FlowForm` / `FlowFormVersion` / `FormFieldCatalogItemDTO` 的 Lombok getter/setter 未生成，且 `log.warn` 被解析为 MyBatis `org.apache.ibatis.logging.Log` 签名；`clean compile` 与显式 `-Dmaven.compiler.proc=full` 复跑仍失败。本轮未改该服务。 |
| 2026-06-29 | 补修：流程设计器字段权限矩阵可点击 | Vitest 定向测试 | `/bin/zsh -lc "source ~/.nvm/nvm.sh && nvm use v20.19.0 >/dev/null && pnpm --dir forge-admin-ui exec vitest run src/components/flow-designer/panel/__tests__/FormPermissionConfig.spec.js src/components/flow-designer/converter/__tests__/user-task-parser-permissions.spec.js src/components/flow-designer/converter/__tests__/json-to-bpmn.spec.js"` | 通过 | 3 个测试文件、29 个用例通过；覆盖 `visible/editable` 与 `readable/writable` 新旧字段冲突、BPMN 读写序列化。 |
| 2026-06-29 | 补修：流程设计器字段权限矩阵可点击 | 空白/冲突检查 | `git diff --check` | 通过 | 无空白错误。 |
| 2026-06-29 | 补修：流程设计器字段权限矩阵可点击 | 前端生产构建 | `/bin/zsh -lc "source ~/.nvm/nvm.sh && nvm use v20.19.0 && NODE_OPTIONS=--max-old-space-size=8192 pnpm --dir forge-admin-ui build"` | 通过 | 构建耗时约 2m；仅有既有 dynamic/static import chunking、CSS `//` 注释和组件命名冲突 warning。 |
| 2026-06-29 | 补修：节点抽屉字段权限保存后丢失 | Vitest 定向测试 | `/bin/zsh -lc "source ~/.nvm/nvm.sh && nvm use v20.19.0 >/dev/null && pnpm --dir forge-admin-ui exec vitest run src/components/flow-designer/__tests__/DingFlowDesigner.spec.js src/components/flow-designer/panel/__tests__/FormPermissionConfig.spec.js src/components/flow-designer/converter/__tests__/user-task-parser-permissions.spec.js src/components/flow-designer/converter/__tests__/json-to-bpmn.spec.js"` | 通过 | 4 个测试文件、45 个用例通过；新增覆盖“打开节点抽屉后未点抽屉保存，直接全局保存/发布时 getXML 先提交草稿”。测试日志有既有 stub/API mock warning。 |
| 2026-06-29 | 补修：节点抽屉字段权限保存后丢失 | 空白/冲突检查 | `git diff --check` | 通过 | 无空白错误。 |
| 2026-06-29 | 补修：节点抽屉字段权限保存后丢失 | 前端生产构建 | `/bin/zsh -lc "source ~/.nvm/nvm.sh && nvm use v20.19.0 && NODE_OPTIONS=--max-old-space-size=8192 pnpm --dir forge-admin-ui build"` | 通过 | 构建耗时约 1m33s；仅有既有 dynamic/static import chunking、CSS `//` 注释和组件命名冲突 warning。 |
| 2026-06-29 | 补修：待办/已办审批表单运行时字段权限不生效 | Vitest 定向测试 | `/bin/zsh -lc "source ~/.nvm/nvm.sh && nvm use v20.19.0 >/dev/null && pnpm --dir forge-admin-ui exec vitest run src/utils/__tests__/field-permissions.spec.js src/components/flow-designer/panel/__tests__/FormPermissionConfig.spec.js src/components/flow-designer/converter/__tests__/user-task-parser-permissions.spec.js src/components/flow-designer/converter/__tests__/json-to-bpmn.spec.js src/components/flow-designer/__tests__/DingFlowDesigner.spec.js"` | 通过 | 5 个测试文件、49 个用例通过；覆盖字段权限 JSON 字符串解析、新旧键优先级、空数组不阻断后续权限源。测试日志有既有 Naive UI stub/API mock warning；补充单跑 `src/utils/__tests__/field-permissions.spec.js` 4 个用例通过。 |
| 2026-06-29 | 补修：待办/已办审批表单运行时字段权限不生效 | 后端 generator 模块编译 | `env JAVA_HOME=/opt/homebrew/Cellar/openjdk@17/17.0.13/libexec/openjdk.jdk/Contents/Home PATH=/opt/homebrew/Cellar/openjdk@17/17.0.13/libexec/openjdk.jdk/Contents/Home/bin:$PATH mvn -pl forge-framework/forge-plugin-parent/forge-plugin-generator -am compile -DskipTests` | 通过 | 仅有既有 deprecation/unchecked warning。 |
| 2026-06-29 | 补修：待办/已办审批表单运行时字段权限不生效 | 空白/冲突检查 | `git diff --check` | 通过 | 无空白错误。 |
| 2026-06-29 | 补修：待办/已办审批表单运行时字段权限不生效 | 前端生产构建 | `/bin/zsh -lc "source ~/.nvm/nvm.sh && nvm use v20.19.0 && NODE_OPTIONS=--max-old-space-size=8192 pnpm --dir forge-admin-ui build"` | 通过 | 最后一次构建耗时约 1m39s；仅有既有 dynamic/static import chunking、CSS `//` 注释和组件命名冲突 warning。 |
| 2026-06-29 | 补修：点击节点表单资产导致权限配置消失 | Vitest 定向测试 | `/bin/zsh -lc "source ~/.nvm/nvm.sh && nvm use v20.19.0 >/dev/null && pnpm --dir forge-admin-ui exec vitest run src/components/flow-designer/panel/__tests__/ApproverConfig.spec.js src/components/flow-designer/panel/__tests__/FormPermissionConfig.spec.js"` | 通过 | 2 个测试文件、5 个用例通过；新增覆盖点击当前表单资产后保留已有字段权限，并为新字段补默认可见/可编辑权限。 |
| 2026-06-29 | 补修：点击节点表单资产导致权限配置消失 | 空白/冲突检查 | `git diff --check` | 通过 | 无空白错误。 |
| 2026-06-29 | 补修：点击节点表单资产导致权限配置消失 | 前端生产构建 | `/bin/zsh -lc "source ~/.nvm/nvm.sh && nvm use v20.19.0 && NODE_OPTIONS=--max-old-space-size=8192 pnpm --dir forge-admin-ui build"` | 通过 | 构建耗时约 1m35s；仅有既有 dynamic/static import chunking、CSS `//` 注释和组件命名冲突 warning。 |
