...
### 19.9 业务域三级目录与子树应用展示补充

- 业务域树支持继续新增子目录，形成“父业务域 -> 子业务域 -> 菜单目录 -> 应用菜单”的层级。
- 业务域保存时必须禁止把父级指向自身或后代，避免领域树和菜单目录形成循环引用。
- 发布低代码应用时应按业务域父链递归解析目录父级，自动把当前业务域目录挂到上级业务域目录下。
- 应用总览选中父业务域时，列表应展示该业务域及其所有子业务域的应用，而不是只看直系 `domain_id`。
- 节点操作应提供“新增子目录”和“编辑领域”，新增子目录时自动回填当前节点为父级。

### 19.10 业务域三级目录实施结论

- `LowcodePublishService` 现在会按业务域父链递归解析目录父级，发布时生成或更新的领域目录会落到正确的上级目录下。
- `MenuRegisterAdapterImpl` 支持领域目录在父级变化时自动迁移，避免重复创建同名目录。
- `LowcodeAppService` 的总览分页已改为按当前领域及子树领域取数，选中父业务域时不会出现“父节点空白、子节点有数据却看不到”的错觉。
- 前端应用总览侧边栏已增加“新增子目录 / 编辑领域”动作，新增子目录时自动继承父级。
- 本轮验证已完成：后端主应用相关模块 compile 通过，前端 `pnpm build` 通过，`git diff --check` 通过。真实菜单落库与浏览器回显验证因未启动后端/数据库/前端 dev server 记录为跳过项。

### 19.11 应用总览业务套件树纠偏

- 用户截图确认目标页面是 `forge-admin-ui/src/views/app-center/index.vue` 的应用总览，不是低代码构建器 `ai_lowcode_domain` 页面。
- 应用总览真实业务域来源是 `ai_business_suite`，因此新增 `parent_id` 字段和索引，并在 `BusinessSuiteDTO/VO/SummaryVO` 中透出父级。
- `BusinessSuiteService` 负责业务套件父级循环校验、子业务域删除拦截和业务套件菜单目录递归解析；访问入口菜单同步复用该递归解析能力。
- 应用总览左侧业务域列表按 `parentId` 渲染成树，节点三点菜单提供“新增子目录 / 编辑信息 / 启停 / 删除”。
- 选中父业务域时，前端计算当前业务域及所有子业务域 `suiteCode`，对象和入口查询通过 `suiteCodes` 聚合返回父子业务域数据。
- 本轮验证已完成：`git diff --check`、应用总览相关 Vue 文件 ESLint、后端 generator/admin 编译均通过。未启动数据库、后端服务或浏览器，真实 Flyway 执行、菜单落库和页面点击回显仍需环境可用时补验。
